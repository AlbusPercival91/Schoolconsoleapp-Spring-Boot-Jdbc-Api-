...
public class SchoolDAO {
  ...
  public List<Group> findGroupsWithLessOrEqualsStudents(int students) {
    String sql = "SELECT groups.group_id, groups.group_name, COUNT(students.student_id) AS number_of_students\n"
        + "            FROM school.groups LEFT JOIN school.students ON groups.group_id = students.group_id\n"
        + "            GROUP BY groups.group_id, groups.group_name HAVING COUNT(students.student_id) <= " + students
        + ";";
